...
define("E_VAL_URL_CHECK_FAILED","Please provide a valid url address");
...
class FormValidator 
{
	...
	function printErrors($plus_class = "")
	{
		echo $this->returnErrors($plus_class);
	}
	
	function returnErrors($plus_class = "")
	{
		$err_msg = "";
		//var_dump($this->error_hash);
		if(count($this->error_hash) > 0)
		{
			foreach($this->error_hash as $err_k=>$err_v)
				$err_msg .= $err_v."<br />";
			
			$err_msg = "<div class='error ".$plus_class."'>" . $err_msg . "</div>";
		}
		return $err_msg;	
	}
			...
			case "regexp":
						{
							if(isset($input_value) && strlen($input_value)>0)
							{
								echo preg_match("$command_value",$input_value);
